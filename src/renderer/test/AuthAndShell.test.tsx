...
describe('SettingsModal and BottomBar', () => {
  ...
  it('shows the server version returned by the readiness health check', async () => {
    ...
    vi.mocked(window.system.listProviders).mockResolvedValueOnce({ source: 'gateway', providers: [] })
    vi.mocked(window.fetch).mockImplementation((url) => {
      if (url.toString().endsWith('/health/ready')) {
        return Promise.resolve({ ok: true, status: 200, json: vi.fn().mockResolvedValue({ status: 'ready', version: '14.0.0' }) } as unknown as Response)
      }
